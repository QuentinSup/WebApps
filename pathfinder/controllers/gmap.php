...
class gmap extends dwBasicController {
	...
	/**
	 * Retourne les informations sur l'endroit geolocalise
	...
	public function geoloc(dwHttpRequest &$request, dwHttpResponse &$response, dwModel &$model) {
		...
		$GMAP_API_KEY = dw::App() -> getProperty('GMAP_API_KEY');
		
		$precision = request::get('precision', "country");

		$responseData = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?result_type='.$precision.'&latlng='.$request -> Path('lat').','.$request -> Path('lng').'&key='.$GMAP_API_KEY);
